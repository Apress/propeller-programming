...
'    repeat j from 0 to 511
'      UARTS.DEC(0, logBuf[j])
'      UARTS.PUTC(0, 13)
...
STEIM org 0
  ...
  mov _clogBufPtrSav, _clogBufPtr
...
:mainLoop
  ...
  ' log the addr of packbuf
  mov t0, _cpackbufPtr
  mov _clogVal, _cpackBufPtr
  call #LOG
  ' log the addr of comprcodebuf
  mov t0, _cpackbufPtr
  mov _clogVal, _ccomprcodebufPtr
  call #LOG
...
:loopns
  ...
  ' If this is the last iteration, DON'T WRITE/INCREMENT
  ' ccomprcode is written again in :done... so it
...
INIT_LOG
  mov _clogIdx, #0
  mov _clogBufPtr, _clogBufPtrSav
INIT_LOG_ret ret

LOG
  wrlong _clogVal, _clogBufPtr
  add _clogBufPtr, #4
  add _clogIdx, #1
  test _clogIdx, _clogMaxIdx wz
  if_nz jmp #:logdone
  mov _clogIdx, #0
  mov _clogBufPtr, _clogBufPtrSav
...
_clogMaxIdx long LOGLEN-1 ' 0 to loglen-1
_clogIdx long 0
_clogVal long 0

_clogIdxPtr long 0
_clogBufPtr long 0
_clogBufPtrSav long 0
