...
PUB MAIN | d, t0, dt, nc, j
    ...
    ' fill sampsbuf with evenly spaced nums...
    repeat j from 0 to nsamps-1
      sampsBuf[j] := j * 50000

    ' except for last 2 samps...
    sampsBuf[126] := $AB
    d := -42
    sampsBuf[127] := $AB + d 
...
PRI TEST_THAT_128_SAMPS_PROPERLY_COMPRESS_AND_DECOMPRESS | t0, t1, j, nc, ns, sav[128]
    nsamps := 128

    ' generate 128 random numbers. Make sure they are in the lower 3 bytes only
    ' per my spec that the dynamic range of my numbers is 24 bits...
    j := 0
...

{
    ...
    ' clear the unused part of packbuf
...
{   
    ...
    ' check that the right number were decompressed
    ' and that the decompressed number equals the input (saved) number
    t0 := (ns == nsamps)
    repeat j from 0 to ns-1
        t1 := (sampsBuf[j] == sav[j])
	' print out if there is an err
        if t1 <> TRUE
            UARTS.PUTC(DEBUG, 13)
            UARTS.DEC(DEBUG, j)
            UARTS.PUTC(DEBUG, COMMA)
